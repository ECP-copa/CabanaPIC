...
begin_initialization {
  ...
  double nx        = 1;    // Global resolution in the x direction
  double ny        = 32;    // Global resolution in the y direction
  double nz        = 1; //32;     // Global resolution in the z direction
  double nppc      = 50; //125; //800; //200; //2048; //1024; //128;    // Average number of macro particles per cell (both species combined!)
  ...
  repeat ( Ne ) {
    ...
    double x = 0.5*Lx; //uniform2( gy0, gy1 , seed );
    double z = 0.5*Lz; //uniform2( gz0, gz1 , seed );
   // double x = uniform( rng(0), xmin, xmax );
   // double y = uniform( rng(0), ymin, ymax );
   // double z = uniform( rng(0), zmin, zmax );
   // double x = uniform( rng(0), gx0, gx1 );
   // double y = uniform( rng(0), gy0, gy1 );
   // double z = uniform( rng(0), gz0, gz1 );
   // n1 = normal(rng(0),v0e,vthex);
