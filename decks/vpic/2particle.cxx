...
begin_initialization {
  ...
  double n0      = 1.0; //0.5;    //  Background plasma density
  ...
  double nx        = 1000; //640; //320; //160;    // Global resolution in the x direction
  double ny        = 1;    // Global resolution in the y direction
  double nz        = 1; //32;     // Global resolution in the z direction
  double nppc      = 1; //125; //800; //200; //2048; //1024; //128;    // Average number of macro particles per cell (both species combined!)
  ...
  double Npe = n0*Lx*Ly*Lz;    // Number physical electrons.
  double Npi = Npe;            // Number of physical ions in box

  double Ne = 2; //two particles only
 ...
 // exit(1);
  ...
  num_step             = 100000; //20000; //10000; //1200; // int(tauwpe/(wpe*dt));
  ...
  global->fft_ex_interval    = 1;
  global->part_loc_interval    = 1;
  ...
  float xp1 = 0.6; //1.5625e-3; //0.303125; //0.30625;
  float xp2 = 0.8; //-0.303125; //-0.30625;
  float xpm = 0.5*(xp1+xp2);
  sim_log( "Loading fields" );
  
  // set_region_field( x<xp1, -0.2, 0, 0,                    // Electric field
  //                   0, 0, 0 ); // Magnetic field
  // set_region_field( x==xp1, 0.3, 0, 0,                    // Electric field
  //                   0, 0, 0 ); // Magnetic field
  // set_region_field( x>xp1&&x<xp2, 0.8, 0, 0,                    // Electric field
  //                   0, 0, 0 ); // Magnetic field
  // set_region_field( x==xp2,0.3, 0, 0,                    // Electric field
  //                   0, 0, 0 ); // Magnetic field
  // set_region_field( x>xp2, -0.2, 0, 0,                    // Electric field
  //                   0, 0, 0 ); // Magnetic field
    ...
    double phi[_nx+2], Ex[_nx+2];
    
     // for (int i=0; i<_nx+2; i++){
     //   //       double xp = _x0 + _dx*i;
     //   double xc = _x0 + _dx*(i-0.5);
     //   if(xc>xp1&&xc<xp2)
     ...
     //   double xc = _x0 + _dx*(i-0.5);
     //   Ex[i] = (phi[i-1] - phi[i+1])/(2.0*_dx) - (xc-0.5)*wi;
     // }
     // Ex[0] = Ex[_nx];
     // Ex[_nx+1] = Ex[1];
     ...
     for (int i=0; i<_nx+2; i++){
       double xn = _x0 + _dx*(i-1);
       //       double xp = _x0 + _dx*(i);
       if(xn>=xp1&&xn<=xp2)
	 phi[i] = (1.0-xn)*xp1*wi + xn*(1.0-xp2)*wi + (xn*xn-xn)*wi;
	 //     	 phi[i] = (xp1+(1.0-xp1-xp2)*xn +xn*xn-xn)*wi;
       else if(xn<xp1)
	 phi[i] = (1.0-xp1)*xn*wi + xn*(1.0-xp2)*wi + (xn*xn-xn)*wi;
     	 //phi[i] = (xn*(2.0-xp1-xp2) +xn*xn-xn)*wi;
       else if(xn>xp2)
	 phi[i] = (1.0-xn)*xp1*wi + xp2*(1.0-xn)*wi + (xn*xn-xn)*wi;
     	 //phi[i] = (xp1+xp2-(xp1+xp2)*xn +xn*xn-xn)*wi;
      
       //       printf("%d %e %e ", i, xc,phi[i]);       
     }
     for (int i=1; i<_nx+1; i++){
       Ex[i] = (phi[i] - phi[i+1])/(_dx);
     }
     Ex[0] = Ex[_nx];
     Ex[_nx+1] = Ex[1];
     ...
     for (int i=0; i<_nx+2; i++){
       ...
       //       printf("%e %e %e %e %e\n", xn,xc,phi[i],Ex[i], (Ex[i]-Ex[i-1])/_dx);
     }
...
#define rand_float(min, max) (min + (max-min)*rand()/RAND_MAX)
  ...
  //  repeat ( Ne ) {
    //double x = (ip+0.5)*dxp;
    ip++;
    double x = xp1; //uniform2( xmin, xmax , seed );
    double y = 0; //uniform2( -a, a , seed );
