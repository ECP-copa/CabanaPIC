...
class Initializer {
  ...
  static void initialize_params(size_t _nc = 16, size_t _nppc = 16)
	    ...
	    //1D
            Parameters::instance().NX_global = 1;
            ...
            Parameters::instance().num_steps = 3000;
            ...
            Parameters::instance().len_y_global = 3.1415926; //6.28318530717959; //default_grid_len;
            ...
            Parameters::instance().len_y = Parameters::instance().len_y_global; //default_grid_len;
            Parameters::instance().len_z = Parameters::instance().len_z_global; //default_grid_len;
        ...
        static void initialize_particles( particle_list_t particles,size_t nx,size_t ny,size_t nz, real_t dxp, size_t nppc, real_t w)
        {
		    ...
		    //test y
                    ...
                    real_t na = 1e-2*sin(6.28318530717959*((x+1.0+pre_ghost*2)/(2*nx)));
                    

                    real_t gam = 1.0/sqrt(1.0-v0*v0);
                    velocity_x.access(s,i) = sign * v0*gam *(1.0-na*sign); //0;
                    velocity_y.access(s,i) = 0;
                    velocity_z.access(s,i) = 0;  //sign * v0 *gam*(1.0+na*sign);
		    //if(pre_ghost*2==ny) velocity_x.access(s,i) = velocity_x.access(s,i)*(1.0-0.01*sign);
                };
